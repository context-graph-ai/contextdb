...
impl RedbPersistence {
    ...
    pub fn open(path: &Path) -> Result<Self> {
        let db = redb::Database::open(path).map_err(Self::storage_error)?;
        Ok(Self { db })
    }

    pub fn flush_data(&self, ws: &WriteSet) -> Result<()> {
        let write_txn = self.db.begin_write().map_err(Self::storage_error)?;
        ...
        for (table, row) in &ws.relational_inserts {
            ...
            let mut redb_table = write_txn.open_table(table_def).map_err(Self::storage_error)?;
    ...
    pub fn flush_table_meta(&self, name: &str, meta: &TableMeta) -> Result<()> {
        let write_txn = self.db.begin_write().map_err(Self::storage_error)?;
        {
            let mut meta_table = write_txn.open_table(META_TABLE).map_err(Self::storage_error)?;
    ...
    pub fn remove_table_meta(&self, name: &str) -> Result<()> {
        let write_txn = self.db.begin_write().map_err(Self::storage_error)?;
        {
            let mut meta_table = write_txn.open_table(META_TABLE).map_err(Self::storage_error)?;
            let key = Self::meta_key(name);
            meta_table.remove(key.as_str()).map_err(Self::storage_error)?;
        }
        write_txn.commit().map_err(Self::storage_error)?;
        Ok(())
    }
    ...
    pub fn load_all_table_meta(&self) -> Result<HashMap<String, TableMeta>> {
        let read_txn = self.db.begin_read().map_err(Self::storage_error)?;
