...
pub trait AnnT {
    ...
    /// path is the directory that will be used. It must be created beforehand (we do not create in order to enforce attention)
    ...
    D: Distance<T> + Send + Sync,
{
    ...
    fn file_dump(&self, path: &Path, file_basename: &str) -> anyhow::Result<String> {
        info!(
            "Hnsw::file_dump: dumping in dir {:#?} with (unique) basename : {}",
            path, file_basename
        );
        //
