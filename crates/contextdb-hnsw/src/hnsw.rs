...
use std::collections::{BTreeMap, binary_heap::BinaryHeap};
...
impl<T: Clone + Send + Sync> PartialEq for PointWithOrder<'_, T> {
    fn eq(&self, other: &Self) -> bool {
        self.dist_to_ref.total_cmp(&other.dist_to_ref) == Ordering::Equal
            && self.point_ref.p_id == other.point_ref.p_id
            && self.point_ref.origin_id == other.point_ref.origin_id
    } // end eq
}
...
impl<T: Clone + Send + Sync> Ord for PointWithOrder<'_, T> {
    fn cmp(&self, other: &Self) -> Ordering {
        self.dist_to_ref
            .total_cmp(&other.dist_to_ref)
            .then_with(|| self.point_ref.p_id.cmp(&other.point_ref.p_id))
            .then_with(|| self.point_ref.origin_id.cmp(&other.point_ref.origin_id))
    } // end cmp
}
...
impl LayerGenerator {
    pub fn new(max_nb_connection: usize, maxlevel: usize) -> Self {
        Self::new_with_rng(max_nb_connection, maxlevel, StdRng::from_os_rng())
    }

    pub fn new_with_seed(max_nb_connection: usize, maxlevel: usize, seed: u64) -> Self {
        Self::new_with_rng(max_nb_connection, maxlevel, StdRng::seed_from_u64(seed))
    }

    fn new_with_rng(max_nb_connection: usize, maxlevel: usize, rng: rand::rngs::StdRng) -> Self {
...
impl<'b, T: Clone + Send + Sync> PointIndexation<'b, T> {
    ...
    fn new_with_rng(
        ...
        let layer_g = LayerGenerator::new_with_rng(max_nb_connection, max_layer, rng);
        PointIndexation {
            max_nb_connection,
            max_layer,
...
impl<'b, T: Clone + Send + Sync, D: Distance<T> + Send + Sync> Hnsw<'b, T, D> {
    ...
    fn new_with_rng(
        ...
        let layer_indexed_points = PointIndexation::<T>::new_with_rng(
            max_nb_connection,
            adjusted_max_layer,
            max_elements,
            rng,
    ...
    fn select_neighbours(
        ...
        if extend_candidates {
            let mut candidates_set = BTreeMap::<PointId, Arc<Point<T>>>::new();
