//! Same-DB cross-thread progress under a registered trigger callback.
...
//! Drop/close while parked, and the panic-freedom suite's regression guards.

use crate::common_tracing::{
    T30_COUNT_ENABLED, T30_LAST_WARN_FIELDS, T30_WARN_COUNT, T37_COUNT_ENABLED, T37_TRACING_COUNT,
...
///
/// The previous helper polled a process-wide commit-tick counter from a sibling
/// thread and reported "engine stalled" if no tick arrived inside a wall-clock
/// window. That is a false-positive signal under workspace-parallel CPU
/// pressure. Individual deadlock-guard tests still assert their own explicit
/// engine timeout behavior independently.
...
// with the panic-freedom suite's `t37_*` siblings; both suites' counters dispatch from one
// `set_global_default`.

// `EnvGuard` (above) is exported via `pub(crate)` so the panic-freedom suite's t37
...
fn t30_01_sustained_writer_flood_50_threads_commits_under_lens7_baseline() {
    run_with_timeout(|| {
        ...
        // 51st party: observer just synchronises the barrier (mirrors a downstream evidence reader thread shape).
...
fn t30_05_sustained_writer_flood_n32_with_no_op_trigger_all_commit() {
    run_with_timeout(|| {
        ...
        assert_eq!(
            ...
            "no writer may observe same-DB B2 typed-Err under the wait-and-proceed contract; saw {observed_typed_err}"
        );
        assert_eq!(
            committed.load(AtomicOrdering::SeqCst),
            n_writers * commits_per_writer
...
fn t30_14_b1_cron_active_same_db_returns_callback_active_cron_immediately() {
    run_with_timeout(|| {
        ...
        // Load-bearing "immediately, not engaging a wait primitive" proof: the
        // B1 cron path returns the typed Cron error from
        // `assert_public_tx_control_cross_thread_allowed_for_surface` BEFORE it
        // can reach `wait_for_same_db_trigger_callback_idle` (the sole
        // incrementer of `wait_observed`), so that counter must not move across
        // this begin(). Replaces a wall-clock elapsed bound.
        let wait_before = db
            .trigger_progress_telemetry_snapshot_for_test()
            .wait_observed;
        let result = db.begin();
        match result {
            ...
            other => {
                panic!("expected B1 (Cron) typed-Err immediately, got {other:?}")
            }
        }
        let snap = db.trigger_progress_telemetry_snapshot_for_test();
        assert_eq!(
            snap.wait_observed, wait_before,
            "B1 must return immediately, not engage a wait primitive; wait_observed moved \
             {wait_before}->{}; snap={snap:?}",
            snap.wait_observed
        );
...
fn t30_15b_schema_agnosticity_wait_holds_mid_cascade() {
    run_with_timeout(|| {
        ...
        // Bounded poll with 1 s timeout — under the correct wait-and-proceed
        // contract the counter increments before Thread C blocks.
...
fn t30_16_drop_while_callback_parked_no_panic_close_releases_after_callback_exits() {
    run_with_timeout(|| {
        ...
        let db_b = db.clone();
        let close_handle = thread::spawn(move || db_b.close());

        // Deterministic replacement for a 50ms sleep-and-probe: close() is the
        // only same-DB waiter, so wait until its park registers on wait_observed
        // instead of sleeping and hoping it reached the wait point.
        let close_parked = wait_until_trigger_wait_observed(&db, Duration::from_secs(2));
        ...
        let close_result = close_handle.join().unwrap();
        assert!(
            close_parked,
            "close() must engage the same-DB trigger wait while the callback is parked"
        );
        assert!(
            !close_pre_release,
...
fn t30_16d_sql_autocommit_dml_does_not_rewait_inside_outer_execute_operation() {
    run_with_timeout(|| {
        ...
        // Attribution order matters: close() starts while the writer is STILL
        // plugin-blocked, so the first wait_observed increment can only be
        // close's park — a regressed writer that re-waits cannot reach the
        // counter before its plugin releases below. (Deterministic replacement
        // for a 50ms sleep-and-probe.)
        let db_close = db.clone();
        let close_probe = thread::spawn(move || db_close.close());
        let close_park_observed = wait_until_trigger_wait_observed(&db, Duration::from_secs(2));
        assert!(
            close_park_observed,
            "close() must park on the same-DB trigger wait while the callback is parked"
        );
        assert!(
        ...
        // Exactly one same-DB wait across the whole scenario: close's park.
        // A regressed writer re-waiting inside its outer execute would raise
        // this to 2.
        assert_eq!(
            db.trigger_progress_telemetry_snapshot_for_test()
                .wait_observed,
            1,
            "the released autocommit writer must not re-wait; close is the sole same-DB waiter"
        );
    })
...
fn t30_18_deadlock_guard_healthy_contention_no_tracing_warn_emitted() {
    ...
    run_with_timeout(|| {
        ...
        for w in 0..n_writers {
            ...
            let commit_attempts = commit_attempts.clone();
            handles.push(thread::spawn(move || {
                writer_barrier.wait();
                for s in 0..commits_per_writer {
                    let tx = match db.begin() {
                    ...
                    commit_attempts.fetch_add(1, AtomicOrdering::SeqCst);
                    db.commit(tx).expect("commit");
                    committed.fetch_add(1, AtomicOrdering::SeqCst);
                }
            }));
        ...
        let overlap_deadline = Instant::now() + Duration::from_secs(30);
        let mut overlap_observed = false;
        while Instant::now() < overlap_deadline {
            if typed_err_observed.load(AtomicOrdering::SeqCst) > 0
                || db.trigger_progress_telemetry_snapshot_for_test().wait_observed > 0
            {
                overlap_observed = true;
                break;
            }
            // Third healthy schedule: writers can stall on the commit path itself, before the
            // instrumented trigger-dispatch wait, so wait_observed never ticks. This arm is
            // event-based, no clock: each writer marks commit_attempts immediately before
            // db.commit, and no commit can complete while the first callback is held — so two
            // or more marked entries with zero completions means a second writer is provably
            // inside the commit path overlapping the hold. The product contract itself is
            // established by the final all-commits-land / zero-warn assertions.
            if commit_attempts.load(AtomicOrdering::SeqCst) >= 2
                && committed.load(AtomicOrdering::SeqCst) == 0
            {
                overlap_observed = true;
                break;
            }
        ...
        // Under the wait-and-proceed contract every commit lands
        // (no typed-Err observed). Under the pre-wait-and-proceed baseline this assertion fails
        // because typed-Err is observed and committed < n_writers * commits_per_writer.
        assert_eq!(
            committed.load(AtomicOrdering::SeqCst),
            n_writers * commits_per_writer,
            "every writer must commit under the wait-and-proceed contract; committed={}, typed_err_observed={}",
            committed.load(AtomicOrdering::SeqCst),
...
fn t30_19_cron_then_trigger_sequential_phases_typed_err_then_wait() {
    run_with_timeout(|| {
        ...
        // Deterministic replacement for a 50ms sleep-and-hope: wait until Thread
        // C's begin() actually parks on the trigger wait (registered by
        // wait_observed) before releasing the callback. Phase 1's cron typed-Err
        // does not touch wait_observed, so this park is the only contributor.
        let phase2_park_observed = wait_until_trigger_wait_observed(&db, Duration::from_secs(2));
        done_trigger.wait();
        ...
        let _ = db.rollback(tx);
        assert!(
            phase2_park_observed,
            "Phase 2: begin() must park on the trigger wait before callback release"
        );
        ...
        // wait-and-proceed contract's wait primitive engagement only.)
        let snap = db.trigger_progress_telemetry_snapshot_for_test();
        assert!(
            snap.wait_observed >= 1,
...
// Sole callers are the nightly-tiered t30_20 / t30_20_release, so this helper
// is gated with them to stay out of the default `-D warnings` build.
#[cfg(feature = "nightly-tests")]
fn run_panic_freedom_begin_iterations(key_base: u128, label: &'static str) {
    ...
    for iteration in 0..1_000 {
        ...
        // begin() under the wait-and-proceed contract waits and proceeds; under
        // the pre-wait-and-proceed baseline it returns typed-Err.
...
fn t30_22_source_order_priority_b1_unrelated_trigger_unchanged() {
    run_with_timeout(|| {
        ...
        // Mirrors t30_14: B1 cron typed-Err must return immediately, not engage
        // the wait primitive. A regression that routes B1 through the
        // wait-and-proceed gate would increment `wait_observed` on db; the
        // immediate typed-Err path never enters the wait loop. Replaces a
        // wall-clock elapsed bound.
        let wait_before = db
            .trigger_progress_telemetry_snapshot_for_test()
            .wait_observed;
        let result = db.begin();
        ...
        assert_eq!(
            snap.wait_observed, wait_before,
            "B1 cron typed-Err must return immediately, not engage the trigger wait primitive; \
             wait_observed moved {wait_before}->{}; snap={snap:?}",
            snap.wait_observed
        );
...
fn t30_02_execute_same_db_b2_waits_then_proceeds() {
    run_with_timeout(|| {
        ...
        // Thread B: db.execute("INSERT ...") must wait and proceed under the
        // wait-and-proceed contract.
...
fn t30_panicking_callback_wakes_parked_waiter() {
    run_with_timeout(|| {
        ...
        // Bounded 1 s timeout — under the correct wait-and-proceed contract the counter increments
...
fn t30_cron_b1_immediate_typed_err_remains_unparked() {
    run_with_timeout(|| {
        ...
        // "Immediately, not parked" is proven by the wait counter: the B1 cron
        // path returns the typed error before entering the same-DB wait loop,
        // so `wait_observed` must not move. Replaces a wall-clock elapsed bound.
        let wait_before = db
            .trigger_progress_telemetry_snapshot_for_test()
            .wait_observed;
        let result = db.begin();
        ...
        assert_eq!(
            snap.wait_observed, wait_before,
            "cron B1 must return immediately, not park; wait_observed moved {wait_before}->{}; \
             snap={snap:?}",
            snap.wait_observed
        );
...
fn t30_16_release() {
    run_with_timeout(|| {
        ...
        assert!(
            close_parked,
            "release: close() must engage the same-DB trigger wait while callback parked"
        );
        assert!(
            !close_pre_release,
...
#[test]
#[serial]
fn t30_24_cross_db_independence_emits_no_warn() {
    // Relocated from trigger_concurrency_panic_freedom.rs (t37_cross_db, 2026-07-18):
    // its two t37 siblings were begin-surface copies of t30_17/t30_18 and were
    // deleted; this cross-DB case is the one assertion that existed nowhere else.
    // Cross-DB independence emits ZERO tracing events; the deadlock warn is
    // reserved for local same-DB wait timeouts.
    t30_install_global_subscriber();
    t37_reset();
    run_with_timeout(|| {
        // db_x: a parked trigger callback holds its firing thread.
        let entered_x = Arc::new(Barrier::new(2));
        let done_x = Arc::new(Barrier::new(2));
        let db_x = Arc::new(Database::open_memory());
        db_x.execute("CREATE TABLE t (id UUID PRIMARY KEY)", &empty())
            .unwrap();
        db_x.execute("CREATE TRIGGER tr_x ON t WHEN INSERT", &empty())
            .unwrap();
        let entered_cb = entered_x.clone();
        let done_cb = done_x.clone();
        db_x.register_trigger_callback("tr_x", move |_db, _ctx| {
            entered_cb.wait();
            done_cb.wait();
            Ok(())
        })
        ...
        // db_y: unrelated trigger-active DB, never fired.
        let db_y = Arc::new(Database::open_memory());
        db_y.execute("CREATE TABLE t (id UUID PRIMARY KEY)", &empty())
            .unwrap();
        db_y.execute("CREATE TRIGGER tr_y ON t WHEN INSERT", &empty())
            .unwrap();
        db_y.register_trigger_callback("tr_y", move |_db, _ctx| Ok(()))
            .unwrap();
        db_y.complete_initialization().unwrap();

        let fire_x = fire_trigger_in_thread(db_x.clone(), 0x3024);
        entered_x.wait();
        ...
        let db_y_probe = db_y.clone();
        let result = thread::spawn(move || db_y_probe.begin()).join().unwrap();
        let tx = result.expect("unrelated DB-Y begin must proceed");
        db_y.rollback(tx).expect("rollback DB-Y tx");

        let engine_events = T37_TRACING_COUNT.load(AtomicOrdering::SeqCst);
        // Positive control: prove the counter is live before asserting zero.
        tracing::warn!(
            target: "contextdb_engine::acceptance_positive_control",
            "trigger callback wait exceeded deadlock guard"
        );
        ...
        assert_eq!(
            engine_events, 0,
            "engine emitted {engine_events} tracing event(s) on cross-DB independence path"
        );
    })
    .expect("t30_24 cross-DB deadlocked");
}

/// Default-gate witness for "a HEALTHY same-DB wait emits no deadlock warn".
/// The stress journeys proving this at scale (t30_18/_release) moved to the
/// nightly tier; without this sibling, a regression that warns on every
/// healthy wait (instead of only on guard timeouts, t30_17) would pass the
/// per-commit gate. Deterministic t30_02 shape: one parked callback, one
/// same-DB writer provably waiting (wait_observed advances), zero warns.
#[test]
#[serial]
fn t30_28_healthy_same_db_wait_emits_no_warn() {
    t30_install_global_subscriber();
    t37_reset();
    run_with_timeout(|| {
        let entered = Arc::new(Barrier::new(2));
        ...
        let fire = fire_trigger_in_thread(db.clone(), 0x3028);
        entered.wait();
        let mut done_guard = BarrierReleaseGuard::armed(done.clone());

        // Count engine tracing events across the whole healthy-wait window.
        T37_TRACING_COUNT.store(0, AtomicOrdering::SeqCst);
        T37_COUNT_ENABLED.store(true, AtomicOrdering::SeqCst);

        let db_writer = db.clone();
        let writer = thread::spawn(move || db_writer.begin());
        // Fresh in-memory DB: wait_observed starts at zero, so >= 1 proves
        // THIS writer engaged the healthy wait.
        assert!(
            wait_until_trigger_wait_observed(&db, Duration::from_secs(10)),
            "the same-DB writer must engage a healthy wait while the callback is parked"
        );
        ...
        let tx = writer
            .join()
            .unwrap()
            .expect("healthy same-DB begin must proceed after the callback completes");
        db.rollback(tx).expect("rollback probe tx");
        fire.join().unwrap().unwrap();
        ...
        assert_eq!(
            engine_events, 0,
            "a healthy same-DB wait must not emit the deadlock warn; got {engine_events}"
        );
    })
    .expect("t30_28 healthy same-DB wait deadlocked");
}

#[test]
fn t30_26_completed_callbacks_leave_no_stale_owner_entries() {
    // Deterministic state guard for TriggerCallbackThreadGuard::drop's
    // owner-map cleanup (its `*count > 1` match guards). The 32-writer flood
    // (t30_05) caught a leaked zero-count owner entry only on rare schedules;
    // this asserts the cleaned-up STATE directly — a completed callback must
    // leave the owner map empty. No second thread parked, no clocks, no
    // schedule dependence.
    let db = Arc::new(Database::open_memory());
    let _pause = db.pause_cron_tickler_for_test();
    db.execute("CREATE TABLE t (id UUID PRIMARY KEY)", &empty())
    ...
    db.execute("CREATE TRIGGER tr ON t WHEN INSERT", &empty())
        .unwrap();
    db.register_trigger_callback("tr", move |_db, _ctx| Ok(()))
        .unwrap();
    db.complete_initialization().unwrap();
    assert_eq!(db.trigger_callback_owner_thread_count_for_test(), 0);
    for key in [0x3026u128, 0x3026F] {
        fire_trigger_in_thread(db.clone(), key)
            .join()
            .unwrap()
            .unwrap();
        assert_eq!(
            db.trigger_callback_owner_thread_count_for_test(),
            0,
            "a completed trigger callback must remove its owner-map entry"
        );
    }
}
