...
fn t34_05_dispatcher_thread_initiated_drop_is_deterministic_and_leak_free_across_iterations() {
    ...
    // Leak-freedom across iterations is asserted as STATE inside every
    // iteration (runtime_starts == 1, last_strong_count == 1,
    // dispatcher_released) — the former cross-iteration elapsed-ratio bounds
    // were load-dependent by construction (owner standing rule: no wall-clock
    // assertions) and added no coverage beyond those state checks. The timing
    // fields remain in the child's result line for operator eyes only.
}
