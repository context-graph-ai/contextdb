...
fn f120_contextdb_server_version_prints_binary_and_protocol_version() {
    ...
    // Protocol version is operator-discoverable; require the current numeric value adjacent to the protocol token.
    assert!(
        stdout.contains("protocol_version=7")
            || stdout.contains("protocol_version 7")
            || stdout.contains("PROTOCOL_VERSION 7"),
        "stdout must include `protocol_version=7` (or equivalent) so operators can detect asymmetric upgrades; got: {stdout}"
    );
}
