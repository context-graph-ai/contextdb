...
fn f120_contextdb_server_version_prints_binary_and_protocol_version() {
    ...
    // Protocol version is operator-discoverable; require the literal numeric `5` adjacent to the protocol token.
    assert!(
        stdout.contains("protocol_version=5")
            || stdout.contains("protocol_version 5")
            || stdout.contains("PROTOCOL_VERSION 5"),
        "stdout must include `protocol_version=5` (or equivalent) so operators can detect asymmetric upgrades; got: {stdout}"
    );
}
