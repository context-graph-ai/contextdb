...
//! - The same-DB cross-thread progress suite (`trigger_same_db_progress.rs`): t30_17
//!   asserts exactly one warn on deadlock-guard timeout; t30_18 asserts zero warns on
//!   healthy contention.
//! - The trigger concurrency panic-freedom suite (`trigger_concurrency_panic_freedom.rs`):
//!   t37_* siblings assert zero deadlock-guard warns on healthy and cross-DB paths; one
//!   warn on the deadlock-guard sibling.
//! - The event-bus suite (`event_bus.rs`): t5_33 asserts sink callback panics are emitted as
