...
//!
//! ConcurrentUniqueConstraintEnforcement.
...
fn t15_15_composite_on_conflict_matches_full_natural_key() {
    // REGRESSION GUARD. Consumers that upsert using composite natural keys such
    // as (source_id, content_hash) depend on this. Matching only the first conflict column
...
fn t15_11_rejected_tx_produces_no_subscription_event() {
    ...
    // the channel; try_recv drains deterministically with no timed wait.
