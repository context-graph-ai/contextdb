...
fn t21_15_legacy_nonmonotonic_txids_are_repaired_on_reopen() {
    ...
    let (first_lsn, second_lsn, before_lsn, after_lsn) = {
        let db = Database::open(&path).unwrap();
        db.execute(
            "CREATE TABLE items (id UUID PRIMARY KEY, label TEXT)",
            &empty(),
        ...
        db.execute(
            "INSERT INTO items (id, label) VALUES ($id, 'before')",
            &HashMap::from([("id".to_string(), Value::Uuid(versioned_id))]),
        )
        .unwrap();
        let before_lsn = rx.recv_timeout(Duration::from_secs(2)).unwrap().lsn;
        db.execute(
            "UPDATE items SET label = 'after' WHERE id = $id",
            &HashMap::from([("id".to_string(), Value::Uuid(versioned_id))]),
        )
        .unwrap();
        let after_lsn = rx.recv_timeout(Duration::from_secs(2)).unwrap().lsn;
        db.close().unwrap();
        (first_lsn, second_lsn, before_lsn, after_lsn)
    };
