...
fn t27_01_schedule_fires_inside_tx_and_commits_callback_writes() {
    ...
    .unwrap();
    // Pause the background tickler BEFORE the schedule exists so it never races
    // dispatch; we drive firing deterministically below. A '1 MILLISECONDS'
    // interval is due within the due-now imminent window.
    let _pause = db.pause_cron_tickler_for_test();
    db.execute(
        "CREATE SCHEDULE gc_evidence EVERY '1 MILLISECONDS' TX (gc_evidence_cb)",
        &empty(),
    )
    .unwrap();
    ...
    // Deterministic replacement for a 100ms-poll race against the live tickler.
    // `cron_run_due_now_for_test` returns only AFTER the cron tx has committed,
    // so two successful drives prove the schedule RE-ARMS (fires more than once,
    // not a one-shot) AND that callback writes commit inside the cron tx.
    db.cron_run_due_now_for_test().expect("first cron drive");
    db.cron_run_due_now_for_test().expect("second cron drive");
    let rows = db.scan("gc_log", db.snapshot()).unwrap();
    assert!(
        rows.len() >= 2,
        "schedule must re-arm and fire more than once (not a one-shot); rows={rows:?}"
    );
}
...
fn t27_02_only_due_schedules_dispatch() {
    ...
    db.execute("CREATE TABLE later_log (id UUID PRIMARY KEY)", &empty())
        .unwrap();
    // Pause the tickler before the schedules exist so dispatch is driven
    // deterministically. `soon` at '1 MILLISECONDS' is due within the due-now
    // imminent window; `later` at 60s is far from due.
    let _pause = db.pause_cron_tickler_for_test();
    db.execute(
        "CREATE SCHEDULE soon EVERY '1 MILLISECONDS' TX (soon_cb)",
        &empty(),
    )
    .unwrap();
    db.execute(
    ...
    // Deterministic dispatch: drive due-now once. Only the DUE schedule (soon)
    // dispatches; the far-future `later` (60s) is not due and must not fire.
    // Replaces a 100ms tickler-race poll.
    db.cron_run_due_now_for_test().expect("cron drive");

    let soon_count = db.scan("soon_log", db.snapshot()).unwrap().len();
    let later_count = db.scan("later_log", db.snapshot()).unwrap().len();
    assert!(soon_count >= 1, "the due schedule (soon) must dispatch");
...
";
    ...
    // The reopened db's background tickler must pick up the PERSISTED schedule
    // and fire it. The callback signals per fire from inside the cron tx; wait
    // for TWO signals so the first fire's row and audit are guaranteed committed
    // by the time the second signal arrives (the Nth cron commit is visible once
    // the (N+1)th fire begins). Bounded failure ceiling replaces a 100ms poll.
    let ceiling = Duration::from_secs(10);
    fire_rx
        .recv_timeout(ceiling)
        .expect("persisted schedule did not fire after reopen within ceiling");
    fire_rx
        .recv_timeout(ceiling)
        .expect("persisted schedule did not fire a second time within ceiling");
    ...
    // Safe shutdown that does not race the live tickler: dropping the Database
    // joins the tickler thread via stop_cron_tickler (waiting for any in-flight
    // cron callback), whereas close() deliberately refuses while a cron callback
    // is active on the tickler thread. The Fired audit is persisted per-fire
    // (append_cron_audit persists immediately), so dropping instead of an
    // explicit close loses nothing for the cross-reopen persistence check.
    drop(db);
...
fn t27_09_tickler_shuts_down_on_drop() {
    ...
    let probe = {
        ...
        .unwrap();
        // Wait for the live tickler's first autonomous fire via a definite
        // signal (the counter is incremented before the send, so it is > 0 once
        // the signal is received). Replaces a 100ms-poll race.
        fire_rx
            .recv_timeout(Duration::from_secs(10))
            .expect("background tickler must fire before the drop-side shutdown assertion");
        assert!(
            counter.load(std::sync::atomic::Ordering::SeqCst) > 0,
            "background tickler must fire before the drop-side shutdown assertion"
        );
        // A background worker handle shares the cron state but not the
        // primary's lifetime — it lets us observe the tickler AFTER the drop.
        db.__trigger_progress_worker_handle_for_test()
        // db (the sole Arc) drops here -> Drop -> stop_cron_tickler joins.
    };
    // Deterministic thread-gone assertion (replaces a 1 s "no more fires"
    // wall-clock window): the primary's Drop joined the tickler thread, so
    // the shared runtime holds no handle and no further fire is possible.
    assert!(
        !probe.cron_tickler_running_for_test(),
    ...
    assert_eq!(
        ...
        "fire count must be final once the tickler is joined"
    );
}
...
fn t27_15_scoped_open_does_not_start_persisted_tickler() {
...

    {
        ...
        .unwrap();
        // Deterministic state assertion (replaces a 500 ms wall-clock window):
        // a scoped open must never start the autonomous tickler at all.
        assert!(
            !scoped.cron_tickler_running_for_test(),
            "scoped open must not start the persisted-schedule tickler"
        );
        scoped.close().unwrap();
    }
