...
// =========================================================================
// A9-10: sync_round_trip_with_persistence
// =========================================================================
#[test]
#[ignore = "requires redb persistence"]
fn a9_10_sync_round_trip_with_persistence() {
    // Use temp directories for persistence test
    let _edge_path = format!("/tmp/contextdb_test_edge_{}.db", Uuid::new_v4());
    let _server_path = format!("/tmp/contextdb_test_server_{}.db", Uuid::new_v4());

    // NOTE: Database::open(path) does not exist yet (requires redb).
    // When persistence is implemented, this test will use:
    //   let edge = Database::open(&edge_path);
    //   let server = Database::open(&server_path);
    // For now, use in-memory to define the test structure.
    let edge = Database::open_memory();
    let server = Database::open_memory();
    let params = HashMap::new();
    ...
    // Edge: 100 observations across separate transactions
    ...
    for _i in 0..10 {
        ...
        server
            .insert_row(
                ...
                make_params(vec![
                    ("id", Value::Uuid(id)),
                    ("description", Value::Text("correlation".into())),
                ]),
            )
            .unwrap();
    ...
    // Bidirectional sync: edge -> server, server -> edge
    // Edge must also have the patterns table for server->edge sync
    edge.execute(
        "CREATE TABLE patterns (id UUID PRIMARY KEY, description TEXT)",
        &params,
    )
    ...
    let edge_cs = edge.changes_since(0);
    server
        .apply_changes(
            edge_cs,
            &ConflictPolicies::uniform(ConflictPolicy::InsertIfNotExists),
    ...
    assert_eq!(
        server
            ...
            .len(),
        100,
        "server must have 100 observations"
    );
    assert_eq!(
        edge.scan("patterns", edge.snapshot()).unwrap().len(),
        10,
        "edge must have 10 patterns"
    );

    // With persistence:
    // drop(edge);
    // drop(server);
    // let edge2 = Database::open(&edge_path);
    // let server2 = Database::open(&server_path);
    // assert_eq!(server2.scan("observations", ...).len(), 100);
    // assert_eq!(edge2.scan("patterns", ...).len(), 10);
    //
    // Also verify LSN watermarks persisted:
    // let edge2_lsn = edge2.current_lsn();
    // assert!(edge2_lsn > 0, "edge LSN must survive persistence");
    //
    // Cleanup
