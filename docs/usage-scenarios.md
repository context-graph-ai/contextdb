...
Before diving into SQL, here are concrete scenarios where these problems surface. Every table you'll see below — `decisions`, `observations`, `intentions`, `entities`, `edges`, `digests`, and the rest — is an example schema these walkthroughs define on top of contextdb's own primitives: `STATE MACHINE`, `DAG`, `IMMUTABLE`, `PROPAGATE`, `RETAIN`, vector columns, graph traversal, and sync (see the [README](../README.md)). You declare your own tables and attach whichever of these policies you need; contextdb enforces them. The agentic-memory ontology used throughout is one illustrative consumer schema, not contextdb's model.
